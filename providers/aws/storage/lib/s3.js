...
const ensureSlash = (dir) => {
  return ensureChar(dir, '/');
};

const doWithRetry = (fn, tryCount, delay, cb) => {
  let tryIndex = 0;
...
const compareMultipartETag = (eTag, multipartETag) => {
  return multipartETag.anyMatch(cleanETag(eTag));
};

const getETagCount = (eTag) => {
  const match = (eTag || '').match(/[a-fA-F0-9]{32}-(\d+)$/);
...
const smallestPartSizeFromFileSize = (fileSize) => {
...
const formatOptions = (options) => {
  const opts = {
    s3                         : options.s3Client || new AWS.S3(options.s3Options),
    s3Pend                     : new Pend(),
    s3RetryCount               : options.s3RetryCount || 3,
    s3RetryDelay               : options.s3RetryDelay || 1000,
    multipartUploadThreshold   : options.multipartUploadThreshold || (20 * 1024 * 1024),
    multipartUploadSize        : options.multipartUploadSize || (15 * 1024 * 1024),
    multipartDownloadThreshold : options.multipartDownloadThreshold || (20 * 1024 * 1024),
    multipartDownloadSize      : options.multipartDownloadSize || (15 * 1024 * 1024)
  };

  opts.s3Pend.max = options.maxAsyncS3 || 20;
...
function Client(options) {
  ...
  } else if (this.multipartUploadThreshold > MAX_PUTOBJECT_SIZE) {
    throw new Error('Maximum multipartUploadThreshold is 5GB.');
  } else if (this.multipartUploadSize < MIN_MULTIPART_SIZE) {
    throw new Error('Minimum multipartUploadSize is 5MB.');
  } else if (this.multipartUploadSize > MAX_PUTOBJECT_SIZE) {
...
Client.prototype.uploadFile = function (params) {
  ...
  uploader.getPublicUrl      = () => {
    return getPublicUrl(s3Params.Bucket, s3Params.Key,
      self.s3.config.region, self.s3.config.endpoint);
  };
  uploader.getPublicUrlHttp  = () => {
...
Client.prototype.downloadBuffer = function (s3Params) {
  ...
  function doTheDownload(cb) {
    ...
    request.on('httpHeaders', function (statusCode, headers, resp) {
      ...
      hashCheckPend.go(function (callback) {
        multipartETag.on('end', function () {
          if (multipartETag.bytes !== contentLength) {
            handleError(new Error("Downloaded size does not match Content-Length"));
            return;
